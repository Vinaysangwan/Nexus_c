...
//                           Sound Functions
// #############################################################################
#define NX_SOUND_POOL_SIZE 8

struct NxSound
{
  char path[512];
  float volume;
  ma_sound pool[NX_SOUND_POOL_SIZE];
  int poolInit[NX_SOUND_POOL_SIZE];
};
...
NxSound* sound_load(const char* path)
{
  if (!audioEngineReady) 
  {
    fprintf(stderr, "[nx_audio] Engine not initialised. Call nx_audio_init() first.\n");
    return NULL;
  }
  ...
  if (result != MA_SUCCESS) 
  {
    fprintf(stderr, "[nx_audio] Failed to load sound '%s': %d\n", path, result);
    free(s);
    return NULL;
  }
...
void sound_free(NxSound* sound)
{
  ...
  for (int i = 0; i < NX_SOUND_POOL_SIZE; i++) 
  {
    if (sound->poolInit[i]) 
    {
...
void sound_play(NxSound* sound)
{
  ...
  if (!sound->poolInit[slot]) 
  {
    ...
    if (result != MA_SUCCESS) 
    {
      fprintf(stderr, "[nx_audio] Pool reinit failed for '%s': %d\n", sound->path, result);
      return;
    }
...
NxMusic* music_load(const char* path)
{
  ...
  NxMusic* m = (NxMusic*)calloc(1, sizeof(NxMusic));
  if (!m) return NULL;
  ...
  if (result != MA_SUCCESS) 
  {
    fprintf(stderr, "[nx_audio] Failed to load music '%s': %d\n", path, result);
    free(m);
    return NULL;
  }
